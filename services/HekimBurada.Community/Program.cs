...
builder.AddBaseForgeLogging("community");
...
builder.Services.AddOpenApi(openApi =>
{
    ...
    {
        document.Info.Title = "Community API";
        document.Info.Version = "v1";
        document.Info.Description = "# Community API\n\nBaseForge ile üretilen **community** mikroservisi · _community microservice generated by BaseForge_.\n\n## 🇹🇷 Türkçe\nBu servis **community** bounded context'ini yönetir ve şu kaynaklar için CRUD uçları sunar: **CommunityCategory, Membership, Topic, Comment, Like**. Her kaynak için oluşturma, güncelleme, silme (soft delete), tekil getirme ve listeleme uçları vardır.\n\nMimari: Clean Architecture + CQRS (MediatR). Veri erişimi: EF Core (+ karmaşık sorgular için Dapper). Veritabanı: PostgreSQL (servise özel, `hekimburada_community_db`). Başka servislere referanslar yalnızca ID iledir (cross-DB FK yoktur).\n\n## 🇬🇧 English\nThe **community** microservice owns the **community** bounded context and exposes CRUD endpoints for: **CommunityCategory, Membership, Topic, Comment, Like**. Each resource provides create, update, delete (soft delete), get-by-id and list endpoints.\n\nArchitecture: Clean Architecture + CQRS (MediatR). Data access: EF Core (+ Dapper for complex queries). Database: PostgreSQL (per service, `hekimburada_community_db`). References to other services are ID-only (no cross-DB foreign keys).";
...
{
    options.UsePostgreSQL<CommunityDbContext>(
    ...
    options.EnableRabbitMq(mq =>
    {
        ...
        mq.Subscribe<Community.Integration.DoctorProfileUpdatedEvent>("identity/DoctorProfileUpdated", "community.SyncMembershipOnDoctorProfileUpdated");
    });
});
...
if (app.Environment.IsDevelopment())
{
    ...
    try
    {
        ...
        scope.ServiceProvider.GetRequiredService<CommunityDbContext>().Database.EnsureCreated();
    }
    catch (Exception ex)
    {
    ...
    {
        options.WithTitle("Community API");
...
app.UseBaseForge();
app.MapControllers();
app.MapGrpcService<Community.Grpc.CommunityCategoryGrpcService>();
app.MapGrpcService<Community.Grpc.MembershipGrpcService>();
app.MapGrpcService<Community.Grpc.TopicGrpcService>();
app.MapGrpcService<Community.Grpc.CommentGrpcService>();
app.MapGrpcService<Community.Grpc.LikeGrpcService>();
app.Run();
