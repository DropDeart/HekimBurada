...
builder.AddBaseForgeLogging("messaging");
...
builder.Services.AddOpenApi(openApi =>
{
    ...
    {
        document.Info.Title = "Messaging API";
        document.Info.Version = "v1";
        document.Info.Description = "# Messaging API\n\nBaseForge ile üretilen **messaging** mikroservisi · _messaging microservice generated by BaseForge_.\n\n## 🇹🇷 Türkçe\nBu servis **messaging** bounded context'ini yönetir ve şu kaynaklar için CRUD uçları sunar: **Message**. Her kaynak için oluşturma, güncelleme, silme (soft delete), tekil getirme ve listeleme uçları vardır.\n\nMimari: Clean Architecture + CQRS (MediatR). Veri erişimi: EF Core (+ karmaşık sorgular için Dapper). Veritabanı: PostgreSQL (servise özel, `hekimburada_messaging_db`). Başka servislere referanslar yalnızca ID iledir (cross-DB FK yoktur).\n\n## 🇬🇧 English\nThe **messaging** microservice owns the **messaging** bounded context and exposes CRUD endpoints for: **Message**. Each resource provides create, update, delete (soft delete), get-by-id and list endpoints.\n\nArchitecture: Clean Architecture + CQRS (MediatR). Data access: EF Core (+ Dapper for complex queries). Database: PostgreSQL (per service, `hekimburada_messaging_db`). References to other services are ID-only (no cross-DB foreign keys).";
...
{
    options.UsePostgreSQL<MessagingDbContext>(
...
if (app.Environment.IsDevelopment())
{
    ...
    {
        options.WithTitle("Messaging API");
